...
def scaffold_split(smi_list, num_sup, seed=1):
    ...
    if len(scaffold_id_list) == 1:
        return idx_list_all[:num_sup], idx_list_all[num_sup:]
    ...
    while True:
        ...
        test_idx_list_all = []
        for scaffold, idx_list in scaffold_id_list:
            if len(train_idx_list_all) >= num_sup:
                test_idx_list_all += idx_list
            else:
                train_idx_list_all += idx_list

        if len(test_idx_list_all) >= 3:
            return train_idx_list_all, test_idx_list_all

        seed = seed + try_iter * 10
        random.seed(seed)
        random.shuffle(scaffold_id_list)
...
class pocketscreen(UnicoreTask):
    ...
    def load_few_shot_ood_dataset(self, split, **kwargs):
        mol_data_path = os.path.join(f"{PROJECT_ROOT}/OOD/test_datasets", "oodtest_unit=%_lig.lmdb")
        pocket_data_path = os.path.join(f"{PROJECT_ROOT}/OOD/test_datasets", "oodtest_unit=%_prot.lmdb")
        assay_test_ood = json.load(open(os.path.join(f"{PROJECT_ROOT}/OOD/test_datasets", "oodtest_unit=%.json")))
        act_all = []
        avgact_train = 6.955628350893639
        for assay in assay_test_ood:
            ...
            if self.args.split_method == "random":
                ...
                select_index_train = select_index[:k_shot]
                select_index_test = select_index[k_shot:]
            elif self.args.split_method == "scaffold":
                smi_list = [x["smi"] for x in assay["ligands"]]
                select_index_train, select_index_test = scaffold_split(smi_list, k_shot)

            if split == "train":
                assay["ligands"] = [assay["ligands"][idx] for idx in select_index_train]
            else:
                assay["ligands"] = [assay["ligands"][idx] for idx in select_index_test]
    ...
    def load_few_shot_timesplit(self, split, **kwargs):
        ...
        print("number of assay_test_unseen", len(assay_test_unseen))
        for assay in assay_test_unseen:
            ...
            if self.args.split_method == "random":
                ...
                smi_list = [x["smi"] for x in assay["ligands"]]
                select_index_train, select_index_test = scaffold_split(smi_list, k_shot, self.args.seed)
        ...
        pocket_dataset = self.load_pockets_dataset(pocket_data_path, is_train=split=="train")
        mol_dataset = self.load_mols_dataset(mol_data_path, "atoms", "coordinates", is_train=split=="train")
        dataset = PairDataset(self.args, pocket_dataset, mol_dataset, assay_test_unseen, split, use_cache=True, cache_dir=self.args.data)
